...
def _extract_eig(
    ...
    # Eigenvectors of matting laplacian matrix
        ...
        # # Define the layers
        # linear_layer1 = nn.Linear(feats.shape[1], feats.shape[1])
        # relu = nn.ReLU()
        # linear_layer2 = nn.Linear(feats.shape[1], feats.shape[1])
        #
        # # Define the model as a sequence of layers
        # model = nn.Sequential(linear_layer1, relu, linear_layer2).cuda()
        # projected_feature = model(feats)
        # projected_feature = projected_feature.cuda()
        # W_feat = (projected_feature @ projected_feature.T)
        ## Feature affinities
        W_feat_ds = (feats @ feats.T)
        layer=nn.Linear(pca_comp,pca_comp).cuda()
        feats_arr=feats.cpu()
        # print(feats_arr.shape)
        feats_arr = pca.fit_transform(feats_arr)
        x0 = torch.from_numpy(feats_arr).float()
        x0 = x0.to(device)
        projected_feature= layer(x0)
        projected_feature = projected_feature.cuda()
        W_feat_pcaproj = (projected_feature @ projected_feature.T)
        W_feat=lambda_ds*W_feat_ds + lambda_pcamlp*W_feat_pcaproj
        if threshold_at_zero:
...
def extract_eigs(
    ...
    kwargs = dict(K=K, which_matrix=which_matrix, which_features=which_features, which_color_matrix=which_color_matrix,
                 ...
                 image_downsample_factor=image_downsample_factor, image_color_lambda=image_color_lambda, lapnorm=lapnorm, pca_comp=pca_comp,lambda_ds=lambda_ds, lambda_pcamlp=lambda_pcamlp)
    print(kwargs)
