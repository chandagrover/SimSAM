...
def extract_features(
    ...
    for i, (images, files, indices) in enumerate(pbar):
        ...
        # print(type(W), W)
        # print(type(P), P)
        H_patch, W_patch = H // P0, W // P0
        H_pad, W_pad = H_patch * P0, W_patch * P0
...
def _extract_multi_region_segmentations(
    ...
    output_file = str(Path(output_dir) / f'{id}_resized.png')
    ...
    # print(np.max(segmap))    #14
    # if np.max(segmap) == 1:
    #     segmap = segmap * 255
    # Save dict
    # print(segmap)
    print(segmap)
    Image.fromarray(np.uint8(segmap * 255)).convert('L').save(output_file)
...
def extract_semantic_segmentations(
    ...
    for bbox_dict in tqdm(bbox_list):
        ...
        # if np.max(segmap) == 1:
        #     segmap = segmap * 255
        # print(np.max(semantic_map))
        # if np.max(semantic_map) == 1:
        #     semantic_map = semantic_map * 255
        # Perform mapping
        semantic_segmap = np.vectorize(semantic_map.__getitem__)(segmap)
        # print(np.max(semantic_segmap))
        # if np.max(semantic_segmap) == 1:
        #     semantic_segmap = semantic_segmap * 255
        # Save
        output_file = str(Path(output_dir) / f'{image_id}.png')
        # print(segmap)
        Image.fromarray(np.uint8(semantic_segmap*255)).convert('L').save(output_file)
        print(semantic_segmap)
        # Image.fromarray(semantic_segmap.astype(np.uint8)).convert('L').save(output_file)
        # Image.fromarray(semantic_segmap.astype(np.uint8)*255).convert('L').save(output_file)
...
def _extract_crf_segmentations(
    ...
    # print(np.max(segmap_crf))
    # if np.max(segmap_crf) == 1:
    #     segmap_crf = segmap_crf * 255


    # Save
    Image.fromarray(np.uint8(segmap_crf * 255)).convert('L').save(output_file)
    # print(segmap_crf)
