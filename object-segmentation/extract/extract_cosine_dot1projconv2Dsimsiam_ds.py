...
from lightly.loss import CosineSimilarity
...
class SimSiam(nn.Module):
    def __init__(self):
        super().__init__()
        self.projection_head = SimSiamProjectionHead(token_feat, 128,token_feat)
        self.prediction_head = SimSiamPredictionHead(token_feat, 128, token_feat)
...
def _extract_eig(
    ...
    if which_matrix == 'affinity_torch':
        # W = feats @ feats.T
        W = contrastive_affinity(feats, feats.T)
...
def extract_eigs(
    ...
    kwargs = dict(K=K, which_matrix=which_matrix, which_features=which_features, which_color_matrix=which_color_matrix,
                 ...
                 image_downsample_factor=image_downsample_factor, image_color_lambda=image_color_lambda, lapnorm=lapnorm, epochs=epochs, batch_size=2)
    print(kwargs)
