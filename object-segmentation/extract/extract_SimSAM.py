...
def extract_eigs(
    ...
    kwargs = dict(K=K, which_matrix=which_matrix, which_features=which_features, which_color_matrix=which_color_matrix,
                  normalize=normalize, threshold_at_zero=threshold_at_zero, images_root=images_root,
                  output_dir=output_dir,
                  image_downsample_factor=image_downsample_factor, image_color_lambda=image_color_lambda,
                  lapnorm=lapnorm, batch_size=batch_size, epochs=epochs)
    print(kwargs)
...
def _extract_crf_segmentations(
    ...
    segmap_orig_res[:H_pad,
    :W_pad] = segmap_upscaled  # replace with the correctly upscaled version, just in case they are different
...
def vis_segmentations(
    ...
    for i, (image_path, segmap_path) in enumerate(zip(image_paths, segmap_paths)):
        ...
        blank_segmap_overlay = label2rgb(label=segmap_fullres, image=np.full_like(image, 128),
                                         colors=colors[segmap_label_indices[segmap_label_indices != 0]], bg_label=0,
                                         alpha=1.0)
