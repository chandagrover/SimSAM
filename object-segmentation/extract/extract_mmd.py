...
def mmd_affinity_matrix(x, y, kernel_type='gaussian', sigma=1.0):
    ...
    if kernel_type == 'gaussian':
        ...
        for i in range(n):
            for j in range(i, n):
                x_kernel[i, j] = x_kernel[j, i] = rbf_kernel(x_kernel[i], x_kernel[j], sigma)
        for i in range(m):
            for j in range(i, m):
                y_kernel[i, j] = y_kernel[j, i] = rbf_kernel(y_kernel[i], y_kernel[j], sigma)
        for i in range(n):
            for j in range(m):
                xy_kernel[i, j] = rbf_kernel(x_kernel[i], y_kernel[j], sigma)
    ...
    if kernel_type != 'polynomial' and kernel_type!="rbf":
...
def extract_eigs(
    ...
    kwargs = dict(K=K, which_matrix=which_matrix, which_features=which_features, which_color_matrix=which_color_matrix,
                 ...
                 image_downsample_factor=image_downsample_factor, image_color_lambda=image_color_lambda, lapnorm=lapnorm, kernel_type=kernel_type, sigma=sigma)
    print(kwargs)
...
def _extract_crf_segmentations(
    ...
    index, (image_file, segmap_path) = inp[:100]
