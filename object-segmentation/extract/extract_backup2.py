...
def _extract_crf_segmentations(
    ...
    output_file = str(Path(output_dir) / f'{id}.png')
    output_file_color = str(Path(output_dir_color) / f'{id}_color.png')
    if Path(output_file).is_file():
        print(f'Skipping existing file {str(output_file)}')
        return  # skip because already generated
    if Path(output_file_color).is_file():
        print(f'Skipping existing file {str(output_file_color)}')
        return  # skip because already generated
    ...
    mask = cv2.imread(output_file,0)
    rgb = gray2color(mask, use_pallet='pascal', custom_pallet=None)
    # rgb = cv2.cvtColor(rgb, cv2.COLOR_BGR2RGB)  # becaues cv2 will change color channels before writing
    cv2.imwrite(output_file_color,rgb)
...
def extract_crf_segmentations(
    ...
    utils.make_output_dir(output_dir)
    utils.make_output_dir(output_dir_color)
    fn = partial(_extract_crf_segmentations, images_root=images_root, num_classes=num_classes, output_dir=output_dir, output_dir_color=output_dir_color,
