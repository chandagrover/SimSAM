...
print(image_segmap_crf.shape)
mask = torch.tensor(np.repeat((image_segmap_crf.reshape(1, 1, 512, 512) == object_Category), 3, axis=1)).to(device)
# mask = torch.tensor(np.repeat((image_segmap_crf.reshape(1, 1, 400, 267) == object_Category), 3, axis=1)).to(device)
# print(type(mask))
# print(mask.shape)
...
res_title = results + args.exp_name + '_'+prompt + '_' + filename + '_' + contentname + '.csv'
...
for epoch in range(0, steps + 1):
    ...
    # Normalize tensors to represent probability distributions
    prob_imgdir = F.softmax(img_direction, dim=1)
    prob_textdir = F.softmax(text_direction, dim=1)

    # # Compute KL divergence
    # kldiv_values = []
    # # Iterate over the rows and columns
    # for x, y in zip(prob_imgdir, prob_textdir):
    #     # print(x.shape,y.shape)
    #     kl_div = F.kl_div(x, y, reduction='sum')
    #     kldiv_values.append(kl_div)
    # # print(kldiv_values)
    # kl_divergence = torch.tensor(kldiv_values)
    # # loss_temp = 1 - kl_divergence
    # loss_temp = kl_divergence
    # loss_temp=loss_temp.to(device)
    ...
    # Normalize tensors to represent probability distributions
    prob_globdir = F.softmax(glob_direction, dim=1)

    # Compute KL divergence
    kldiv_values = []
    # Iterate over the rows and columns
    for x, y in zip(prob_globdir, prob_textdir):
        # print(x.shape,y.shape)
        kl_div = F.kl_div(x, y, reduction='mean')
        kldiv_values.append(kl_div)
    # print(kldiv_values)
    kl_divergence = torch.tensor(kldiv_values)
    # loss_glob = 1 - kl_divergence
    loss_glob = kl_divergence
    loss_glob=loss_glob.to(device)
    ...
    if epoch % 100 == 0:
        out_path = output_title + args.exp_name + '_' + prompt + '_' + filename + '_' + contentname  + '.jpg'
        ...
        print(filename, "stylized saved successfully.")
